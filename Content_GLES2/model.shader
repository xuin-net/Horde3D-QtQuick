...
// Contexts
// all 3d model use this context default
context GENERAL
{
...
#include "vertCommon.glsl"

#ifdef _F01_Skinning
        #include "vertSkinning.glsl"
#endif
...
#endif
    mediump vec3 view = normalize( viewerPos - pos.xyz );
    mediump vec3 specColor = vec3(0.1, 0.1, 0.1) * normal * normalize(dot(matSpecParams.rgb, view));
    mediump vec4 nalbedo = albedo * matDiffuseCol + vec4(specColor, 0);
...
void main( void )
{
	...
	gl_Position = viewProjMat * pos;
    //gl_Position = viewProjMat * vec4(lightVec, 1.0);
}
...
void main( void )
{
	...
	if( albedo.a < 0.01 ) discard;
#else
    mediump vec4 albedo = vec4(0.98, 0.98, 0.98, 1.0);
#endif
...
void main( void )
{
    ...
    mediump vec4 vsPos = calcViewPos( pos );
	gl_FragColor.rgb =
