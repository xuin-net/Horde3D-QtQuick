...
DLLEXP bool h3dutLoadResourcesFromDisk( const char *contentDir )
{
    ...
    while( res != 0 )
    {
        ...
        QByteArray data;
        string path = contentDir + resourcePaths[h3dGetResType( res )] + h3dGetResName( res );
        QFile file(path.c_str());
        if (file.exists()){
            if (!file.open(QIODevice::ReadOnly)){
                qDebug()<<"Horde3DUtils Error: Failed to open file. path:" << path.c_str();
                return 0;
            }
            ...
            QString pathQS = QString(path.c_str());
            QString fileName = pathQS.mid(pathQS.lastIndexOf("/") + 1);
//            data = BookEncode::getInstance()->getData(fileName);
        }

//        qDebug() << "加载文件" << path.c_str();
        bufSize = data.size();
        if(bufSize == 0){
            ...
            memcpy(dataBuf, data.constData(), bufSize);
            //Send resource data to engine
            result &= h3dLoadResource( res, dataBuf, bufSize);
            delete[] dataBuf;
        }
